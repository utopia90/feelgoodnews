...
function displayScienceNews(){
    ...
    for (let i = 0; i < sciencePostsArr.length; i++) {
       ...
       <div class="card w-55">
       <div class="card-body">
          <img class="card-img card-image" src=${currSciencePost.thumbnail}
          alt="Card image cap" onerror="this.onerror=null;this.src='/sol.jpg';">   
          <a href=${currSciencePost.url} target="_blank" class="post-link btn btn-primary">Read More</a>
          <h6 class="card-title">${currSciencePost.title}</h6>
       </div>
    </div>
  </div>
       `;
